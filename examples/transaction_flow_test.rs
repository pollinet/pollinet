...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "android")]
    {
        ...
        for (idx, fragment) in fragments.iter().enumerate() {
            println!(
                "   Fragment {}/{}: ID={}, index={}, total={}, data={} bytes",
                idx + 1,
                fragments.len(),
                &fragment.id[..8],
                fragment.index,
        ...
        println!(
            "📊 Outbound queue after dequeue: {} fragments\n",
            outbound_after
        );
        ...
        for (idx, fragment_bytes) in transmitted_fragments.iter().enumerate() {
            ...
            match transport.push_inbound(fragment_bytes.clone()) {
                Ok(_) => {
                    println!(
                        "   📥 Added fragment {}/{} to reassembly buffer",
                        idx + 1,
                        transmitted_fragments.len()
                    );

                    // Check metrics after each fragment
                    let metrics = transport.metrics();
                    println!(
                        "      Fragments buffered: {}, Transactions complete: {}",
        ...
        println!(
            "📊 Received queue size: {} transactions",
            received_queue_size
        );
        ...
        if received_queue_size > 0 {
                ...
                transport.next_received_transaction()
            {
                ...
                println!(
                    "   First 16 bytes: {:02X?}",
                    &reassembled_bytes[..16.min(reassembled_bytes.len())]
                );
                ...
                let mock_signature =
                    "5j7s8K9L2m3n4p5q6r7s8t9u0v1w2x3y4z5a6b7c8d9e0f1g2h3i4j5k6l7m8n9o0p";
                ...
                drop(conf_queue);
                println!(
                    "📊 Confirmation queue size: {} confirmations",
                ...
                eprintln!(
                    "❌ Failed to retrieve transaction (queue reported size > 0 but returned None)"
                );
            }
        } else {
            ...
            eprintln!(
                "   - Fragments buffered: {}",
                final_metrics.fragments_buffered
            );
            eprintln!(
                "   - Transactions complete: {}",
                final_metrics.transactions_complete
            );
