...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, nonce_info) in bundle.nonce_caches.iter_mut().enumerate() {
        if nonce_info.used {
            info!(
                "⏭️  Skipping used nonce {}/{}: {}",
                i + 1,
                total_nonces,
                nonce_info.nonce_account
            );
            continue;
        }
        ...
        match rpc_client.get_latest_blockhash() {
            ...
            Err(e) => {
                ...
                match rpc_client.get_account(&nonce_pubkey) {
                    ...
                    Err(account_err) => {
                        warn!(
                            "   ⚠️  Nonce account not found - may have been closed: {}",
                            account_err
                        );
...
async fn display_nonce_info(
    ...
    let nonce_pubkey = nonce_account
        .parse()
        .map_err(|e| format!("Invalid nonce account pubkey: {}", e))?;
    ...
    match rpc_client.get_account(&nonce_pubkey) {
        Ok(account) => {
            ...
            if account.owner == solana_sdk::system_program::id() && account.data.len() == 80 {
                ...
                warn!(
                    "⚠️  Account exists but is not a valid nonce account: {}",
                    nonce_account
                );
                Ok(false)
            }
