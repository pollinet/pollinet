...
impl BleAdapterBridge {
        ...
        self.adapter.on_receive(Box::new(move |data| {
            ...
            if let Ok(fragment) = serde_json::from_slice::<Fragment>(&data) {
                ...
                tokio::spawn(async move {
                    ...
                    let transaction_fragments = cache_guard
                ...
                if let Ok(text_message) = String::from_utf8(data) {
                    ...
                    tokio::spawn(async move {
                        ...
                        tracing::info!(
                            "📨 Text message received and added to buffer: {}",
                            text_message
                        );
                    });
                } else {
    ...
    pub async fn start_advertising(
        ...
        self.adapter
            .start_advertising(service_uuid, service_name)
            .await
    }
    ...
    pub async fn get_discovered_devices(
        &self,
    ) -> Result<Vec<crate::ble::adapter::DiscoveredDevice>, BleError> {
