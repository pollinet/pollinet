...
mod linux_impl {
    use super::super::adapter::{
        AdapterInfo, BleAdapter, BleError, POLLINET_SERVICE_NAME, POLLINET_SERVICE_UUID,
    };
    use async_trait::async_trait;
    use bluer::{
        adv::Advertisement,
        gatt::local::{
            Application, Characteristic as LocalCharacteristic, CharacteristicRead,
            CharacteristicWrite, Service as LocalService,
        },
    ...
    impl LinuxBleAdapter {
        ...
        async fn start_gatt_server(&self) -> Result<(), BleError> {
            ...
            let app_handle = self
                ...
                .await
                .map_err(|e| {
                    BleError::PlatformError(format!("Failed to serve GATT application: {}", e))
                })?;
            ...
            tracing::info!(
                ...
                POLLINET_TX_CHAR_UUID
            );
            tracing::info!(
                "   📊 Status Characteristic (readable): {}",
                POLLINET_STATUS_CHAR_UUID
            );
        ...
        async fn discover_gatt_services(
            ...
            tracing::info!(
                "🔍 Discovering GATT services for device: {}",
                device.address()
            );
            ...
            let device_services = device
                .services()
                .await
                .map_err(|e| BleError::PlatformError(format!("Failed to get services: {}", e)))?;

            for service in device_services {
                let service_uuid = service.uuid().await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to get service UUID: {}", e))
                })?;
        ...
        async fn setup_gatt_data_reception(
            ...
            for service in device_services {
                let _service_uuid = service.uuid().await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to get service UUID: {}", e))
                })?;

                // Get characteristics for this service
                let characteristics = service.characteristics().await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to get characteristics: {}", e))
                })?;

                for characteristic in characteristics {
                    let char_uuid = characteristic.uuid().await.map_err(|e| {
                        BleError::PlatformError(format!("Failed to get characteristic UUID: {}", e))
                    })?;

                    let properties = characteristic.flags().await.map_err(|e| {
                        BleError::PlatformError(format!(
                            "Failed to get characteristic properties: {}",
                            e
                        ))
                    })?;
                    ...
                    if properties.notify || properties.indicate {
                        ...
                        if let Err(e) = Self::subscribe_to_characteristic(
                            &characteristic,
                        ...
                        {
                            tracing::warn!(
                                "⚠️  Failed to subscribe to characteristic {}: {}",
                                char_uuid,
                                e
                            );
                        } else {
        ...
        async fn subscribe_to_characteristic(
            ...
            // Start notifications
            characteristic.notify().await.map_err(|e| {
                BleError::PlatformError(format!(
                    "Failed to start notifications for characteristic {}: {}",
                    char_uuid, e
                ))
            })?;
            ...
            tokio::spawn(async move {
                tracing::info!(
                    "🎧 Listening for notifications on characteristic: {}",
                    char_uuid
                );
                ...
                if let Ok(notifications) = characteristic_clone.notify().await {
                    ...
                    tracing::warn!(
                        "⚠️  Failed to create notification stream for characteristic: {}",
                        char_uuid
                    );
                }
            });
        ...
        async fn discover_characteristics(
            ...
            let service_characteristics = service.characteristics().await.map_err(|e| {
                BleError::PlatformError(format!("Failed to get characteristics: {}", e))
            })?;
        ...
        async fn find_writable_characteristic(
            ...
            for service in services {
                let current_service_uuid = service.uuid().await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to get service UUID: {}", e))
                })?;
                ...
                for characteristic in characteristics {
                    ...
                    if properties.write || properties.write_without_response {
                        ...
                        let is_standard_bt = char_uuid.as_u128()
                            & 0xFFFFFFFF00000000FFFFFFFFFFFFFFFFu128
                            == 0x0000000000001000800000805F9B34FBu128;

                        if is_standard_bt {
                            tracing::debug!(
                                "⏭️  Skipping standard Bluetooth characteristic: {}",
                                char_uuid
                            );
                            continue;
                        }
                        ...
                        if fallback_char.is_none() {
                            tracing::debug!(
                                "📝 Found writable characteristic: {} in service: {}",
                                char_uuid,
        ...
        async fn get_device_info(
            ...
            if has_pollinet_service {
                ...
                tracing::info!(
                    "✅ Device {} advertises PolliNet service with UUIDs: {:?}",
                    address,
                    service_uuids
                );
                ...
                tracing::debug!(
                    "⚪ Device {} does not advertise PolliNet service (UUIDs: {:?})",
                    address,
                    service_uuids
                );
        ...
        async fn start_bluez_advertising(&self) -> Result<(), BleError> {
            ...
            if !self.adapter.is_powered().await.map_err(|e| {
                BleError::PlatformError(format!("Failed to check adapter power status: {}", e))
            })? {
                tracing::warn!("⚠️  BLE adapter is not powered on, attempting to power on...");
                self.adapter.set_powered(true).await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to power on adapter: {}", e))
                })?;
            ...
            if !self.adapter.is_discoverable().await.map_err(|e| {
                BleError::PlatformError(format!("Failed to check discoverable status: {}", e))
            })? {
                tracing::warn!(
                    "⚠️  BLE adapter is not discoverable, attempting to set discoverable..."
                );
                self.adapter.set_discoverable(true).await.map_err(|e| {
                    BleError::PlatformError(format!("Failed to set discoverable: {}", e))
                })?;
            }
            ...
            for (config_name, advertisement) in configurations {
                ...
                match self.adapter.advertise(advertisement).await {
                    Ok(handle) => {
                        tracing::info!(
                            "✅ Successfully started advertising with {} configuration",
                            config_name
                        );
    ...
    impl BleAdapter for LinuxBleAdapter {
        async fn start_advertising(
            ...
            if service_uuid != POLLINET_SERVICE_UUID {
                return Err(BleError::InvalidUuid(format!(
                    "Expected {}, got {}",
                    POLLINET_SERVICE_UUID, service_uuid
                )));
            }
            ...
            match self.start_bluez_advertising().await {
                ...
                Err(e) => {
                    tracing::warn!(
                        "⚠️  BLE advertising failed, but continuing in simulation mode: {}",
                        e
                    );
        ...
        async fn send_packet(&self, data: &[u8]) -> Result<(), BleError> {
            ...
            for (address, _client_info) in clients_guard.iter() {
                ...
                // Get the device
                let device_address = address.parse::<bluer::Address>().map_err(|e| {
                    BleError::PlatformError(format!("Invalid device address: {}", e))
                })?;
        ...
        async fn connected_clients_count(&self) -> usize {
            ...
            tracing::debug!(
                "📊 Connected clients: {} outbound, {} inbound, {} total",
                outbound_count,
                inbound_count,
                total
            );
        ...
        async fn start_scanning(&self) -> Result<(), BleError> {
            ...
            let discovery_events =
                self.adapter
                    ...
                    .await
                    .map_err(|e| {
                        BleError::PlatformError(format!("Failed to create discovery stream: {}", e))
                    })?;
                            ...
                            if let Ok(device) = adapter_for_task.device(device_id) {
                                ...
                                .await
                                {
                                    Ok(Ok(Some(discovered_device))) => {
                                        ...
                                        devices_guard
                                            .insert(device_id.to_string(), discovered_device);
                                    }
                                    Ok(Ok(None)) => {
                                        tracing::debug!(
                                            "⚪ Device {} does not advertise PolliNet service",
                                            device_id
                                        );
                                    }
                                    Ok(Err(e)) => {
                                        tracing::debug!(
                                            "⚠️ Failed to get device info for {}: {}",
                                            device_id,
                                            e
                                        );
                                    }
                                    Err(_) => {
                                        tracing::debug!(
                                            "⏱️ Timeout getting device info for {}",
                                            device_id
                                        );
                                    }
                                }
        ...
        async fn get_discovered_devices(
            &self,
        ) -> Result<Vec<super::super::adapter::DiscoveredDevice>, BleError> {
            let devices_guard = self.discovered_devices.read().await;
            let devices: Vec<super::super::adapter::DiscoveredDevice> =
                ...
                if is_connected {
                    ...
                    if !clients_guard.contains_key(address) {
                        ...
                        if !services.is_empty() {
                            if let Err(e) = Self::setup_gatt_data_reception(
                                &device,
                                &services,
                ...
                match connect_result {
                    Ok(Ok(())) => {
                        ...
                        if let Ok(is_connected) = device.is_connected().await {
                            if !is_connected {
                                tracing::warn!(
                                    "⚠️  Connection dropped immediately after connecting"
                                );
                        ...
                        .await
                        {
                            Ok(Ok(services)) => {
                                tracing::info!(
                                    "✅ Successfully discovered {} GATT services",
                                    services.len()
                                );
                                services
                            }
                        ...
                        if !services.is_empty() {
                            ...
                            .await
                            {
                                tracing::warn!("⚠️  Failed to set up GATT data reception: {}", e);
                                tracing::warn!(
                                    "   Connection will continue but data reception may not work"
                                );
                            } else {
                    ...
                    Ok(Err(e)) => {
                        ...
                        if error_msg.contains("br-connection-aborted-by-local") {
                            ...
                            tracing::info!(
                                "   • Device is busy or already connected to another device"
                            );
        ...
        async fn write_to_device(&self, address: &str, data: &[u8]) -> Result<(), BleError> {
            ...
            if !device.is_connected().await.map_err(|e| {
                BleError::PlatformError(format!("Failed to check connection status: {}", e))
            })? {
            ...
            if let Some(client_info) = client_info {
                ...
                if let Some((service_uuid, characteristic_uuid)) =
                    find_cached_writable_characteristic(&client_info)
                {
                    tracing::info!(
                        "📝 Using cached characteristic: {} in service: {}",
                        characteristic_uuid,
                        service_uuid
                    );
    ...
    fn find_cached_writable_characteristic(client_info: &ClientInfo) -> Option<(Uuid, Uuid)> {
    ...
    async fn write_to_characteristic(
        ...
        let properties = characteristic.flags().await.map_err(|e| {
            BleError::PlatformError(format!("Failed to get characteristic properties: {}", e))
        })?;
        ...
        if write_with_response {
            characteristic.write(data).await.map_err(|e| {
                BleError::TransmissionFailed(format!(
            ...
            characteristic.write(data).await.map_err(|e| {
                BleError::TransmissionFailed(format!(
                    "Failed to write to characteristic (write without response) {}: {}",
                    char_uuid, e
                ))
            })?;
        }
...
impl crate::ble::adapter::BleAdapter for LinuxBleAdapter {
    async fn start_advertising(
        ...
        _service_uuid: &str,
        _service_name: &str,
    ) -> Result<(), crate::ble::adapter::BleError> {
        Err(crate::ble::adapter::BleError::OperationNotSupported(
            "Linux BLE adapter not available - compile with 'linux' feature".to_string(),
        ))
    }
    ...
    async fn get_discovered_devices(
        &self,
    ) -> Result<Vec<crate::ble::adapter::DiscoveredDevice>, crate::ble::adapter::BleError> {
        Err(crate::ble::adapter::BleError::OperationNotSupported(
            "Linux BLE adapter not available - compile with 'linux' feature".to_string(),
        ))
    }
    ...
    async fn read_from_device(
        &self,
        _address: &str,
    ) -> Result<Vec<u8>, crate::ble::adapter::BleError> {
        Err(crate::ble::adapter::BleError::OperationNotSupported(
            "Linux BLE adapter not available - compile with 'linux' feature".to_string(),
        ))
    }
