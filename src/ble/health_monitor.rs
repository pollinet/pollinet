...
impl MeshHealthMonitor {
    ...
    pub fn record_heartbeat(&self, peer_id: &str) {
        ...
        if let Some(peer) = peers.get_mut(peer_id) {
            ...
            // New peer discovered
            peers.insert(peer_id.to_string(), PeerHealth {
    ...
    pub fn record_latency(&self, peer_id: &str, latency_ms: u32) {
        ...
        if let Some(peer) = peers.get_mut(peer_id) {
            ...
            // Update average
            peer.avg_latency_ms = peer.latency_samples.iter().sum::<u32>() 
                / peer.latency_samples.len() as u32;
    ...
    pub fn get_peer_health(&self, peer_id: &str) -> Option<PeerHealth> {
        ...
        peers.get(peer_id).map(|p| {
            let mut peer = p.clone();
            peer.seconds_since_last_seen = Instant::now()
    ...
    fn update_metrics(&self) {
        ...
        let stale = peers.values().filter(|p| p.state == PeerState::Stale).count();
        let dead = peers.values().filter(|p| p.state == PeerState::Dead).count();
    ...
    fn calculate_health_score(
        ...
        let unhealthy_ratio = (metrics.stale_peers + metrics.dead_peers) as f32 
            / metrics.total_peers as f32;
        ...
        let avg_quality: u8 = peers.values()
            .map(|p| p.quality_score)
            .sum::<u8>() / peers.len() as u8;
    ...
    fn calculate_hop_counts(&self, connections: &HashMap<String, Vec<String>>) -> HashMap<String, u8> {
