...
impl MeshTransport {
    ...
    pub async fn send_text_message(&self, peer_id: &str, message: &str) -> Result<(), BleError> {
        // Get the first available adapter
        let adapters = self.manager.adapters().await?;
        if adapters.is_empty() {
            return Err(BleError::NoAdapter);
        ...
        peripheral.discover_services().await
            .map_err(|_e| BleError::ServiceNotFound)?;
        ...
        peripheral.write(fragment_char, message_bytes, WriteType::WithResponse).await
            .map_err(|e| BleError::TransmissionFailed(e.to_string()))?;
    ...
    pub async fn start_text_listener(&self) -> Result<(), BleError> {
    ...
    pub async fn check_incoming_messages(&self) -> Result<Vec<String>, BleError> {
