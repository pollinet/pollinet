...
pub trait BleAdapter: Send + Sync {
    ...
    async fn connect_to_device(&self, address: &str) -> Result<(), BleError> {
        Err(BleError::OperationNotSupported(
            "Connection not supported on this platform".to_string(),
        ))
    }
    ...
    async fn write_to_device(&self, address: &str, data: &[u8]) -> Result<(), BleError> {
        Err(BleError::OperationNotSupported(
            "Writing to devices not supported on this platform".to_string(),
        ))
    }
    ...
    async fn read_from_device(&self, address: &str) -> Result<Vec<u8>, BleError> {
        Err(BleError::OperationNotSupported(
            "Reading from devices not supported on this platform".to_string(),
        ))
    }
}
