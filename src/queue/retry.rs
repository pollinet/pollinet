...
impl RetryQueue {
    ...
    pub fn pop_ready(&mut self) -> Option<RetryItem> {
        ...
        let ready_key = self.items.keys().find(|&&time| time <= now).copied();
...
pub enum RetryError {
    ...
    MaxAgeExceeded { tx_id: String, age_hours: u64 },
}
...
mod tests {
    ...
    fn create_test_item(id: &str) -> RetryItem {
        RetryItem::new(vec![1, 2, 3], id.to_string(), "test error".to_string())
    }
