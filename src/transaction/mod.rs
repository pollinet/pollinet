...
impl TransactionService {
    ...
    pub async fn submit_to_solana(&self, transaction: &[u8]) -> Result<String, TransactionError> {
        ...
        if let Ok(tx) = serde_json::from_slice::<Transaction>(transaction) {
            let signature = format!(
                "real_signature_{}",
                hex::encode(&tx.message.recent_blockhash.to_bytes()[..8])
            );
        ...
        Err(TransactionError::Serialization(
            "Could not deserialize transaction".to_string(),
        ))
    }
    ...
    async fn get_recent_blockhash_or_nonce(
        &self,
    ) -> Result<solana_sdk::hash::Hash, TransactionError> {
