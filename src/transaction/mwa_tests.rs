...
mod mwa_tests {
    ...
    async fn test_create_unsigned_offline_transaction() {
        ...
        println!("✅ Created unsigned transaction: {} bytes", tx_bytes.unwrap().len());
    }
    ...
    async fn test_get_transaction_message_to_sign() {
        ...
        let unsigned_tx = service.create_unsigned_offline_transaction(
            &sender.pubkey().to_string(),
            &recipient.pubkey().to_string(),
            1_000_000,
            &nonce_authority.pubkey().to_string(),
        ...
        println!("✅ Extracted message to sign: {} bytes", message_bytes.len());
    }
    ...
    async fn test_mwa_flow_without_signing() {
        ...
        let message = service.get_transaction_message_to_sign(&unsigned_tx).unwrap();
